...
## nullスコアの扱い
- null は「データなし」であり「悪い」とも「良い」とも解釈しない
- `overall_score` 算出時に null スコアは除外し、残りのスコアで評価する
- `dissenting_views` にデータ欠損による不確実性を追記する（例: 「調教データなし（training）のため仕上がり状態は不明」）
- 書記が算出した `agreement_score` の割引（データ不足ペナルティ）をそのまま尊重する
